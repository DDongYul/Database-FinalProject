...
def main():
    global iter_num_of_page
    global start_page_url
    ...
    for i in range(0, iter_num_of_page):
        movie_link_list = driver.find_elements_by_css_selector('#old_content > ul > li > a')
        for movie_link in movie_link_list:
            cur.execute('select * from movie where movie_id = %s', url_to_id(movie_link.get_attribute('href')))
            if not cur.fetchone():
        ...
        print('--page into--')
        print(driver.current_url.split('page=')[-1])
